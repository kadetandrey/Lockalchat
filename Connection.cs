...
namespace Chat
{
	...
	class Connection
	{
		...
		{
			IPEndPoint source = new IPEndPoint(userIP, sendPort); 
			IPEndPoint destination = new IPEndPoint(BroadcastIP, receivePort); 

			UdpClient udpSender = new UdpClient(source);
			...
			try
			{
				int count = udpSender.Send(messageBytes, messageBytes.Length, destination);
